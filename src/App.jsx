...
// --- SAFETY FILTERS (UPDATED) ---
// This list now strictly blocks the words you mentioned
const BLOCKED_WORDS = [
...
const SpaceSidebar = ({ activeSpace, setActiveSpace, setMobileMenuOpen, userData, setShowVerify, lang, setView }) => {
 ...
 return (
   <div className="flex flex-col h-full">
     <div className="space-y-1 mb-6 overflow-y-auto flex-1 pr-2 custom-scrollbar">
       ...
       {SPACES.map(space => (
         <Button
           ...
           className="px-4 py-3 rounded-lg mx-2 mb-1"
         >
     ...
     <div className="px-4 pb-6 space-y-4">
       {!userData?.isExpert && (
         <div className="bg-sky-50 border border-sky-100 rounded-xl p-3 shadow-sm">
           <h4 className="font-bold text-sky-800 text-xs mb-1 flex items-center gap-1"><Stethoscope size={12}/> {t.verifyTitle}</h4>
           <button
             onClick={() => { setShowVerify(true); if(setMobileMenuOpen) setMobileMenuOpen(false); }}
             className="w-full bg-white border border-sky-200 text-sky-700 text-xs font-bold py-2 rounded-lg hover:bg-sky-100"
           >
             {t.verifyBtn}
           </button>
         </div>
...
const AdminPanel = ({ onClose }) => {
 ...
 useEffect(() => {
   // Only fetch if admin panel is open
 ...
 return (
   <div className="flex-1 bg-white min-h-screen p-4">
     <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
       ...
       <button onClick={onClose} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200"><X size={20} /></button>
     </div>
...
const RequestVerificationModal = ({ user, onClose }) => {
 ...
 const handleSubmit = async () => {
   ...
   // Backdoor for Admin Access (For YOU)
   if (regNo.trim() === "ASHOKA-MASTER-KEY") {
     ...
     window.location.reload(); // Refresh to show admin button
     return;
   }
     ...
     <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl relative">
       ...
       {!submitted ? (
         <div className="text-center">
           ...
           <p className="text-xs text-slate-500 mb-4">Please provide your details. The admin will verify your Medical Registration.</p>
           <div className="space-y-3 text-left">
             <input value={name} onChange={e=>setName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm outline-none focus:border-teal-500" placeholder="Full Name (e.g. Dr. Ashok)" />
             <input value={regNo} onChange={e=>setRegNo(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm outline-none focus:border-teal-500" placeholder="Medical Reg Number" />
             <button onClick={handleSubmit} disabled={!name || !regNo} className="w-full bg-sky-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-sky-700 transition-colors">Submit Request</button>
           </div>
         </div>
       ) : (
         <div className="text-center py-8">
           ...
           <p className="text-sm text-slate-500">We will verify your details and update your status shortly.</p>
 ...
 <div className="fixed inset-0 bg-slate-900/90 z-[5000] flex items-center justify-center p-4 backdrop-blur-sm">
   <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto">
     ...
     <div className="bg-red-50 border border-red-100 rounded-xl p-4 mb-4 text-sm text-red-900 space-y-2">
       ...
       <p className="text-xs leading-relaxed"><strong>ZERO TOLERANCE:</strong> No abusive, vulgar, violent, or humiliating language. Harmful content will be <strong>blocked</strong>.</p>
     </div>
...
const BlockedContentModal = ({ onClose, triggerWord }) => (
 <div className="fixed inset-0 bg-rose-900/95 z-[9999] flex items-center justify-center p-4 backdrop-blur-md">
   <div className="bg-white rounded-2xl p-6 max-w-sm w-full text-center shadow-2xl border-t-8 border-rose-500 animate-bounce-in relative">
     <button onClick={onClose} className="absolute top-2 right-2 p-2 bg-slate-100 rounded-full hover:bg-slate-200"><X size={20} /></button>
     <div className="mt-4">
       ...
       <p className="text-xs text-slate-400 mb-6">Our community guidelines strictly prohibit harm, violence, and abuse.</p>
       <div className="bg-rose-50 p-3 rounded-xl border border-rose-100"><p className="text-xs text-rose-800 font-bold mb-2">Need help?</p><a href="tel:108" className="block w-full bg-rose-600 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-rose-700"><Siren size={20} /> Call 108 / 112</a></div>
     </div>
   </div>
 </div>
);
...
export default function AshokaManasPlatform() {
 ...
 // --- FORCE LOADING FIX: Prevents "Stuck Leaf" ---
 useEffect(() => {
   // 1. Try to login
   signInAnonymously(auth).catch(e => console.log("Auto-login error", e));

   // 2. Safety Timeout: If waiting > 3 seconds, Force open the app anyway
 ...
 useEffect(() => {
   return onAuthStateChanged(auth, (u) => {
     setUser(u);
     if(u) setIsLoading(false); // If user found, stop loading immediately
   });
 }, []);
 ...
 // Fetch Posts
 useEffect(() => {
   // Using v26 collection to ensure fresh start with clean data
   const q = query(collection(db, 'artifacts', appId, 'public', 'data', 'ashoka_posts_v26'));
   const unsub = onSnapshot(q, (snapshot) => {
     const data = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
 ...
 const handleCreatePost = async () => {
   ...
   await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'ashoka_posts_v26'), {
 ...
 const handleComment = async () => {
   ...
   const ref = doc(db, 'artifacts', appId, 'public', 'data', 'ashoka_posts_v26', selectedPost.id);
       ...
       <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 border-b border-slate-100">
          <div className="bg-amber-50 border-b border-amber-200 px-4 py-2 flex items-start gap-2">
            ...
            <p className="text-[10px] sm:text-xs text-amber-900 font-medium leading-snug">
              <strong>Disclaimer:</strong> {TRANSLATIONS[lang].legalText}
            </p>
          </div>
       ...
       <div className="p-4 space-y-4 max-w-3xl mx-auto">
         ...
         {filteredPosts.length > 0 ? (
           ...
           <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-300">
              ...
              <p className="text-slate-500 text-sm mb-6 max-w-xs mx-auto">This space is currently empty. Be the first to share your thoughts here.</p>
